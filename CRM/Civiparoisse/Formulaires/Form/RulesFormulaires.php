...
// Default constructor pour ne pas permettre une instanciation de la classe depuis l'extérieur
    ...
    public static function validateWebsite($fields)
    {
        ...
        if (is_array($fields)
            && !empty($fields)
            && !empty($fields['web_site'])
            && !filter_var($fields['web_site'], FILTER_VALIDATE_URL)) {
            $errors['web_site'] =
                ts('Merci de corriger l\'adresse du site Internet, '
                    . 'en saisissant bien le http:// ou le https:// au début');
            return $errors;
        }
        return true;
    }
