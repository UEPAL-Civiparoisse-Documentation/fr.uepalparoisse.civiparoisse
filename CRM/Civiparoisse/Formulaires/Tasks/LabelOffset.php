...
class CRM_Civiparoisse_Formulaires_Tasks_LabelOffset extends CRM_Contact_Form_Task_Label
{
  public function buildQuickForm()
  {
    static::buildLabelForm($this);
  }

  public static function buildLabelForm($form)
  {
    $form->add('text','offset',E::ts('label offset'),['value' => 0],true);
    $form->add('checkbox','transpose',E::ts('label transpose'));
    parent::buildLabelForm($form);
  }

  public function addRules(): void
  {
    parent::addRules();
    $this->addFormRule(array('CRM_Civiparoisse_Formulaires_Tasks_LabelOffset', 'validateOffset'));
  }

  public static function validateOffset($fields): bool|array
  {

    $errors = [];
    if (is_array($fields) && array_key_exists('offset', $fields)) {
      $offset = $fields['offset'];

      if (is_null($offset) || !preg_match('/^(([0-9])|([1-9][0-9]+))$/',$offset) || $offset < 0) {
        $errors['offset'] = E::ts('Offset should be a zero or positive integer');
      } else {
        return true;
      }
    } else {
      $errors['offset'] = E::ts('Offset is required');
    }
    return $errors;
  }
  ...
  {
    $values = parent::setDefaultValues();
    $values['offset'] = 0;
    $values['transpose'] = 0;
    return $values;
  }
  ...
  public function postProcess()
  {
    ...
    if (!array_key_exists('offset', $fv) || empty($fv['offset'])){
      $fv['offset'] = 0;
    }
    $locName = NULL;
    ...
    $this->createTransposedLabel($roff, $fv['label_name'], $fv['transpose']);
    CRM_Utils_System::civiExit();
  }

  private function createTransposedLabel(array $contactRows, $format, $transpose)
  {
    $pdf = new CRM_Civiparoisse_PDF_TransposedLabel($format, 'mm');
    $pdf->Open();
    $pdf->AddPage();
