...
{strip}{literal}
  <style type="text/css">
    #civiparoisse-news-feed {
      border: 0 none;
      font-family: inherit; // Stops JQueryUI's attempt to make this section Verdana
    }
    #civiparoisse-news-feed .crm-news-feed-unread .crm-news-feed-item-title {
      font-weight: bold;
    }
    #civiparoisse-news-feed details:not([open]) summary {
      text-overflow: ellipsis;
      text-wrap: none;
      white-space: nowrap;
      overflow: hidden;
    }
    #civiparoisse-news-feed .crm-news-feed-item-preview {
      display: inline;
      color: #8d8d8d;
    }
    #civiparoisse-news-feed details[open] .crm-news-feed-item-preview {
      display: none;
    }
    #civiparoisse-news-feed .crm-news-feed-item-link {
      margin-bottom: 0;
    }
  </style>
{/literal}
  <div id="civiparoisse-news-feed">
    <ul>
      {foreach from=$feeds item="channel"}
        <li class="ui-corner-all crm-tab-button" title="{$channel.description|escape}">
          <a href="#civiparoisse-news-feed-{$channel.name}">{$channel.title}</a>
        </li>
      {/foreach}
    </ul>

    {foreach from=$feeds item="channel"}
      <div id="civiparoisse-news-feed-{$channel.name}">
...
{literal}<script type="text/javascript">
  (function($, _) {
    $(function() {
      $('#civiparoisse-news-feed').tabs();
      var opened = CRM.cache.get('civiparoisseFeed', {});
      $('#civiparoisse-news-feed ul.ui-tabs-nav a').each(function() {
        var
          $tab = $(this),
          href = $tab.attr('href'),
          $content = $(href),
          ...
          $items.each(function () {
            ...
            if ($.inArray(itemKey, opened[key]) < 0) {
              ...
              $(this).one('click', function () {
                ...
                CRM.cache.set('civiparoisseFeed', opened);
              });
            }
          });
          if (count) {
